        ...
        try:
            ...
            with open(output_path, 'w', newline='') as f:
                ...
                for i in range(num_channels):
                    rel_pos = i / max(1, num_channels - 1)
                    
                    if rel_pos <= 0.5:
                        red = 0.996094
                        green = 0.144533 + rel_pos * 1.0
                        blue = max(0, 0.652326 - rel_pos * 1.3)
                    else:
                        adj_pos = (rel_pos - 0.5) * 2
                        red = max(0, 0.996094 - adj_pos * 0.996094)
                        green = max(0, 0.996094 - adj_pos * 0.5)
                        blue = min(0.996094, adj_pos * 0.996094)
                    
                    f.write(f"Ch{i+1},{times_start[i]:.3f},{times_end[i]:.3f},{red:.6f},{green:.6f},{blue:.6f},2\n")
